#include "include/Enemie/EnemieM.hpp"

EnemieM::EnemieM(sf::Texture &a_tileset) :
    Enemie::Enemie(a_tileset)
{
    for(int i=0; i < 1; i++){
        sf::IntRect pos(i*78, 7*72, 78, 72);
...
void EnemieM::Draw(sf::RenderWindow &a_window)
{
    for(Tir &tir : m_TirList){
        tir.Draw(a_window);
    }
...
void EnemieM::Move(sf::Sprite &a_Player)
{
